...
export default function KtaRegistrationForm() {
  const [isPending, startTransition] = useTransition();
  const [isHighContrast, setIsHighContrast] = useState(false);
  const [errorMsg, setErrorMsg] = useState<string | null>(null);
  const [successMsg, setSuccessMsg] = useState<string | null>(null);

  // Sinkronisasi state high-contrast dengan class root <html>
  useEffect(() => {
    const rootHasContrast = document.documentElement.classList.contains('high-contrast');
    setIsHighContrast(rootHasContrast);

    // Observer untuk mendeteksi perubahan class pada <html> secara real-time
  ...
  // Dynamic styling classes based on high-contrast state
        ...
        <div className={`rounded-xl p-6 sm:p-8 ${cardClass}`}>
          ...
          >
            <div className="text-center">
              ...
              <h2 className="text-[1.5em] font-bold mb-4">Pendaftaran Berhasil!</h2>
              <p className="text-[1.125em] mb-6">{successMsg}</p>
              <p className="opacity-80 mb-8">
                Data NIK Anda telah dienkripsi dengan standar AES-256-GCM dan tersimpan aman di sistem.
                Tim verifikasi DPD PKS Pamekasan akan menghubungi Anda melalui WhatsApp.
              </p>
              <button
                type="button"
                onClick={() => setSuccessMsg(null)}
                className={`min-h-[44px] min-w-[44px] px-8 py-3 font-bold rounded transition-all focus:outline-none focus:ring-4 ${btnClass}`}
              >
                Daftar Lagi
              </button>
            </div>
      ...
      <div className={`rounded-xl p-6 sm:p-8 ${cardClass}`}>
        ...
        <form action={handleAction} className="space-y-6">
          ...
          <div className="space-y-2">
            <label htmlFor="nama_lengkap" className={`block ${labelClass}`}>
          ...
          {/* NIK */}
          <div className="space-y-2">
            <label htmlFor="nik" className={`block ${labelClass}`}>
              NIK (Nomor Induk Kependudukan) <span className="text-red-500" aria-hidden="true">*</span>
            </label>
            <input
              type="text"
              id="nik"
              ...
              className={`w-full min-h-[44px] px-4 py-3 rounded-lg outline-none transition-all focus:ring-4 ${inputClass}`}
            />
            <p className="text-sm opacity-70" id="nik-hint">
              NIK akan dienkripsi sebelum disimpan. Data Anda aman.
            </p>
          </div>

          {/* Nomor WhatsApp */}
          <div className="space-y-2">
            <label htmlFor="nomor_whatsapp" className={`block ${labelClass}`}>
            ...
            <input
              ...
              className={`w-full min-h-[44px] px-4 py-3 rounded-lg outline-none transition-all focus:ring-4 ${inputClass}`}
            />
            <p className="text-sm opacity-70" id="whatsapp-hint">
              Digunakan untuk konfirmasi verifikasi oleh tim DPD.
            </p>
          </div>

          {/* Kecamatan */}
          <div className="space-y-2">
            <label htmlFor="kecamatan" className={`block ${labelClass}`}>
          ...
          <div className="pt-4">
            ...
            >
              {isPending ? 'Mengenkripsi & Mengirim Data...' : 'Daftarkan KTA Saya'}
            </button>
          </div>
        </form>
        ...
        <div className="mt-8 pt-6 border-t border-current opacity-70">
          <p className="text-sm text-center leading-relaxed">
            ...
            algoritma AES-256-GCM dan tidak dapat dibaca oleh pihak yang tidak berwenang.
          </p>
        </div>
      </div>
    </div>
